...
function buildPrompt(
  ...
  const base = `You are ${shop.name}'s assistant drafting a short customer message.
Write the message in the language identified by BCP 47 code "${language}".
  ...
  return callWithRetry(async () => {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model,
        messages: [
          {role: "system", content: buildPrompt(kind, job, shop, shop.language)},
        ],
